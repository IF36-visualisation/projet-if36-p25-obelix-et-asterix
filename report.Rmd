...
## Exploration initiale des données et corrélations
...
En analysant les ventes selon le genre des clients (Walmart) et selon le segment de clientèle (Superstore), on observe plusieurs tendances intéressantes.

D’une part, les femmes semblent dépenser légèrement plus que les hommes, bien que la dispersion des dépenses soit relativement similaire entre les deux groupes. On remarque également plusieurs points aberrants dans les deux catégories, suggérant l’existence de clients ayant des comportements d’achat particulièrement élevés. Cela soulève des questions sur l’influence potentielle d’autres facteurs, comme le type de produit acheté, sur ces écarts de dépenses.

D’autre part, en observant les ventes selon le segment de clientèle (Consumer, Corporate, Home Office), on note que les médianes sont proches, ce qui indique une répartition relativement équilibrée. Toutefois, le segment "Consumer" semble générer un peu plus de ventes élevées, avec une concentration légèrement supérieure de valeurs extrêmes. Globalement, la distribution des ventes reste assez similaire entre les trois segments, sans écart majeur.

Ces premières observations suggèrent donc que ni le genre ni le segment ne constituent à eux seuls des facteurs décisifs, mais ils peuvent jouer un rôle complémentaire dans certaines tendances d’achat.
...
## Question 8 : Est-ce qu’un long délai de livraison fait perdre de l’argent ?

**Données utilisées** : `Ship Date`, `Order Date`, `Profit` (Superstore)
...
## Question 12 : Quels États font le plus de ventes ?

**Données utilisées** : `State`, `Sales` (Superstore)
...
## Question 19 : Est-ce que les gens notent mieux s’ils paient plus ?

**Données utilisées** : `rating`, `total` (Walmart)
