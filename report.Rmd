...
<details> 
<summary><strong> Question : Quels produits sont beaucoup achetés ?
</strong></summary>
...
En observant les graphiques des ventes par jour de la semaine, plusieurs constats se dégagent pour les deux jeux de données :

**Superstore**

- Lundi et vendredi affichent les plus hauts niveaux de ventes, dépassant les 420 000 $.

- Mercredi est le jour le plus faible, avec un volume de ventes nettement en dessous des autres jours.

- Samedi et dimanche présentent aussi des montants élevés, mais légèrement inférieurs à ceux du lundi et du vendredi.

Interprétation :

- Superstore vend principalement à des entreprises (B2B).

- Les commandes sont fortes en début de semaine (lundi) pour anticiper l’activité, et en fin de semaine (vendredi) pour se préparer à la suivante.

- Mercredi marque un creux logique : la majorité des besoins ont déjà été couverts.

**Walmart**

- Samedi est le jour où les ventes sont les plus élevées, suivi par mardi.

- Dimanche reste aussi assez élevé, bien que légèrement en dessous du samedi.

- Lundi à vendredi, les ventes sont relativement stables mais plus basses que le week-end.

Interprétation :

- Walmart vend aux particuliers (B2C).

- Les gens achètent plus le week-end : ils ont plus de temps libre pour faire leurs courses.

- En semaine, les ventes sont plus faibles, car les clients sont occupés par le travail ou l’école.
</details>
...
Le graphique présente la note moyenne donnée par les clients en fonction du montant total payé :
...
À partir du graphique, on peut tirer les observations suivantes :

Les pics de ventes se situent :

- autour de 13h (34 723 $) → pause déjeuner.

- vers 19h (39 700 $) → sortie du travail.

Les ventes sont plus faibles :

- Entre 16h et 17h, période intermédiaire entre travail et retour à la maison.

- À partir de 20h, les achats diminuent fortement.

Interprétation :

- À midi, les clients profitent de leur pause pour faire quelques courses rapides.

- En soirée, ils s’arrêtent au magasin en rentrant du travail.

- Tôt le matin et tard le soir, l’activité baisse car les clients ne sont pas disponibles (sommeil, travail, repas).
</details>
...
# Bar chart avec couleur conditionnelle
ggplot(profit_sous_cat, aes(x = reorder(Sub.Category, Profit_total), y = Profit_total, fill = Profit_total > 0)) +
  geom_col() +
  coord_flip() +
  scale_fill_manual(values = c("red", "steelblue"), labels = c("Non rentable", "Rentable")) +
  labs(
    title = "Superstore : Profit total par sous-catégorie",
    x = "Sous-Catégorie",
    y = "Profit total ($)",
    fill = "Rentabilité"
  ) +
